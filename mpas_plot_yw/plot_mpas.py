...
def plot_mpas_worker(task_info):
    ...
    try:
        ...
        if fieldname[-1] == 'u':
            print('Converting edge-U with angleEdge', grid_source)
            ds = xr.open_dataset(
                 grid_source, engine="netcdf4", mask_and_scale=False,    # avoid unwanted fill-value masking on integer connectivity arrays
                                      decode_cf=False)         # skip CF decoding if coordinates are being mangled
            plot_data = plot_data * np.cos(ds['angleEdge'])
            ds.close()
